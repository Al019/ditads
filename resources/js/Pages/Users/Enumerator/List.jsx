...
const List = () => {
  ...
  const handleAdd = () => {
    setError({ last_name: null, first_name: null, email: null })
    post(route('admin.user.add.enumerator'), {
      onSuccess: () => {
        reset()
        handleOpen()
        toast.success("Enumerator added successfully.")
      },
    });
  }
  ...
  const handleSearch = (e) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      router.get(route('admin.user.enumerator'), { search: value }, { preserveState: true });
    }, 1000);
  };
        ...
        <Table>
          ...
          <TableBody>
            {enumerators.data.length > 0 ? (
              enumerators.data.map((enumerator, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <Badge variant={enumerator.status === 'active' ? 'default' : 'destructive'}>
                      <span className='capitalize'>{enumerator.status}</span>
                    </Badge>
                  </TableCell>
                </TableRow>
              ))
          ...
          <div className='flex justify-end'>
            <div>
              <Pagination>
                <PaginationContent>
                  <PaginationItem>
                    <PaginationPrevious onClick={() => handlePage(enumerators.prev_page_url)} className="cursor-pointer" />
                  </PaginationItem>
                  <PaginationItem>
                    <PaginationNext onClick={() => handlePage(enumerators.next_page_url)} className="cursor-pointer" />
                  </PaginationItem>
                </PaginationContent>
              </Pagination>
            </div>
